...
### v0.2 (после Stage 1 экспериментов)

Полный экспериментальный лог: [`docs/reports/stage1_v0.md`](reports/stage1_v0.md) (32 прогона). Ниже — итоговые архитектурные поправки, уже отражённые в тексте секций 6, 7, 9, 10, 26–28 ниже. v0.1 описывала непроверенную гипотезу; v0.2 фиксирует то, что реально подтвердилось экспериментально на Stage 1 (dense continuous autoencoder, один и тот же аудиоклип, затем реальный LibriSpeech).

Главный найденный баг: **continuous-time state непригоден как единственный transport channel для полнополосного сигнала.** Медленно затухающее состояние (`α → 1`) admits только `1-α` новой информации за шаг — если через него обязан пройти весь сигнал, оптимизатор компенсирует это раздуванием downstream весов, что и даёт взрывы градиента и (что важнее) роботизированное, неестественное звучание. Общий вывод записан отдельно и универсально для будущих проектов Manifestro: [`../../lessons-continuous-time-dynamics.md`](../../lessons-continuous-time-dynamics.md).

Конкретные изменения:

1. **Candidate self-recurrence убран** (было: `u_t = tanh(W_x x_t + W_hh h_{t-1})`, стало: `u_t = tanh(W_x x_t)`). Память и так существует через `α_t h_{t-1}`; второй recurrent-механизм внутри candidate не давал измеримой пользы ни на encoder-, ни на decoder-стороне, а на decoder-стороне вдобавок был главной причиной насыщения hidden state (`|h| → 1`) и роботизации звука. См. секцию 7.
2. **Encoder больше не имеет cross-timescale connections** — fast/mid/slow полностью независимы. На encoder-стороне cross-talk ни разу не понадобился в успешно обученной конфигурации. Decoder cross-connections сохранены (отключение дало нейтральный результат — не хуже, не лучше). См. секцию 9.
3. **Добавлен direct/instantaneous path в encoder** (новое, ранее в спеке отсутствовало): `z_t = W_x f_t + Σ gate_i · P_i(h_t^i)`, где `W_x` — почти identity-проекция необработанных frontend-фич, `gate_i` — обучаемые скаляры с малой инициализацией (~0.1). Continuous dynamics больше не транспортируют сигнал напрямую — они добавляют context/memory поверх мгновенного представления. См. секцию 10.
4. **Добавлен direct skip в decoder** (новое): `y_t = to_output(norm(fused)) + skip_gate · W_skip(u_t)`. В отличие от encoder, `skip_gate` — **фиксированная константа (0.05), не обучаемый параметр**: свободный gate систематически схлопывался к нулю (optimizer предпочитает дешёвый skip-путь вместо того, чтобы обучать более сложную temporal-ветку) — это ломало как стабильность, так и естественность звучания. См. секцию 26.
5. Rate/quantization/event-gate секции (13 и далее) пока не тронуты экспериментально — вся работа Stage 1 шла на dense-конфигурации (каждый шаг — «событие»), поправки выше относятся только к continuous dynamics encoder/decoder.
...
**Обновление v0.2:** в валидированном Stage 1 encoder эти три ветви полностью независимы — без cross-timescale communication (секция 9 описывает механизм, но на encoder-стороне он не используется). Также fused latent — это не единственный выход encoder: рядом существует прямой instantaneous path от frontend-фич (секция 10).
...
**Обновление v0.2:** candidate ниже больше не зависит от `h_{t-1}` — терм `W_h h_{t-1}` убран из кандидата (но не из decay/update — memory по-прежнему полностью реализуется через `α_t h_{t-1}` в конце этой секции). Эксперименты (`docs/reports/stage1_v0.md`, Run 14/28) не показали пользы от второго recurrent-пути внутри candidate; на decoder-стороне он вдобавок оказался основной причиной насыщения hidden state (`|h|→1`) и нестабильности/роботизации звука. Формула ниже — уже исправленная (v0.1 содержала `W_h h_{t-1}+b` внутри `tanh`).
...
**Обновление v0.2:** этот механизм подтверждён только для decoder. На encoder-стороне валидированная (Stage 1) конфигурация не использует cross-timescale communication вообще — fast/mid/slow полностью независимы (секция 6). На decoder-стороне отключение cross-connections дало нейтральный результат (не хуже, не лучше — `docs/reports/stage1_v0.md`, Run 30), поэтому они сохранены в decoder по умолчанию. Формулы ниже описывают decoder-конфигурацию.
...
**Обновление v0.2 — ключевое архитектурное дополнение.** В v0.1 fused latent `z_t` строился *только* из continuous states (`h_t^F/h_t^M/h_t^S`) — то есть continuous dynamics были единственным transport channel для всего сигнала. Эксперименты (`docs/reports/stage1_v0.md`, Run 11–19) показали: это структурно не работает — медленно затухающее состояние (`α→1`) admits слишком мало новой информации за шаг (`1-α`), чтобы пропустить через себя полнополосный acoustic-сигнал; попытка всё равно это сделать приводила к градиентным взрывам и, что важнее, к неестественному/роботизированному звучанию. Общий вывод (применим за пределами AEVUM) записан в [`../../lessons-continuous-time-dynamics.md`](../../lessons-continuous-time-dynamics.md).

Исправление: добавлен **прямой instantaneous path** от frontend-фич `f_t`, который идёт в `z_t` параллельно continuous states, а не вместо них:
...
* `W_x` — почти identity-инициализированная проекция (при совпадении размерностей `f_t` и `z_t`), так что в начале обучения `z_t ≈ f_t`, и continuous branches ничего не обязаны нести с первого шага;
* `P_F/P_M/P_S` — линейные проекции каждой ветви в пространство `z_t`;
* `g_F/g_M/g_S` — обучаемые скаляры, инициализируются небольшими (~0.1), так что каждая ветвь обязана «заслужить» свой вклад через обучение, а не полагаться на него с самого начала.

Continuous dynamics в этой схеме перестают быть транспортным каналом для сигнала — они становятся **memory/context поверх мгновенного представления**:

$$
\text{representation} = \text{instantaneous innovation} + \text{temporal context}
$$

Старая формулировка (`z_t = W_z RMSNorm([h_t^F;h_t^M;h_t^S])`, `z_t \in \mathbb{R}^{512}`) сохраняется ниже для истории, но **не используется** в валидированной Stage 1 конфигурации — там `z_t \in \mathbb{R}^{384}` (совпадает с размерностью frontend-фич, что и позволяет identity-инициализацию `W_x`).
...
**Обновление v0.2:** как и в encoder (секция 7), терм `W_h d_{t-1}` убран из candidate — экспериментально (`docs/reports/stage1_v0.md`, Run 28) именно он оказался основной причиной насыщения decoder hidden state (`|d|→1`) и связанных с этим взрывов градиента и роботизированного звучания. Убирание этого терма (при сохранении memory через `α_t^D d_{t-1}` ниже) снизило пиковый градиент почти в 12 раз и полностью убрало насыщение состояния.
...
**Обновление v0.2 — direct skip.** Как и в encoder (секция 10), fused decoder state — не единственный путь к выходу. Добавлен прямой skip от входного события `u_t` к `y_t`, той же природы, что и instantaneous path в encoder:

$$
y_t=
W_y[d_t^F;d_t^M;d_t^S]
+
g_{skip} \cdot W_{skip}(u_t)
$$

**Критическое отличие от encoder:** `g_{skip}` — **фиксированная константа (0.05 в текущей Stage 1 конфигурации), не обучаемый параметр.** Эксперимент (`docs/reports/stage1_v0.md`, Run 26) показал: если сделать `g_{skip}` обучаемым, optimizer систематически схлопывает его к ~0 — skip-путь даёт быстрый и дешёвый градиент, а temporal branch сложнее оптимизировать, поэтому чем меньше `g_{skip}`, тем меньше градиента получает temporal branch, что делает её ещё хуже обученной, и это самоусиливающийся коллапс (gate-collapse). При зафиксированном `g_{skip}=0.05` temporal branch вынуждена реально обучаться — это одновременно даёт лучший loss *и* решает проблему роботизированного звучания, которую не удавалось решить никаким подбором обучаемого gate.
